...
public class ProxySharingScaler implements AutoCloseable {
    ...
    private void reconcile() {
        ...
        int minimumSeats = dynamicMinimumSeatsAvailable != null ? dynamicMinimumSeatsAvailable : specExtension.minimumSeatsAvailable;
        ...
        debug(String.format("Status: %s, Unclaimed: %s + PendingDelegate: %s - PendingDelegating: %s = %s -> minimum: %s",
            ...
            pendingDelegatingProxies.size(), num, minimumSeats));
        ...
        if (num < minimumSeats) {
            ...
            long numToScaleUp = minimumSeats - num;
            ...
            lastReconcileStatus = ReconcileStatus.ScaleUp;
            lastScaleUp = Instant.now();
        } else if ((num - minimumSeats) >= specExtension.seatsPerContainer) {
            long numToScaleDown = (num - minimumSeats) / specExtension.seatsPerContainer;
            if (numToScaleDown <= 0) {
                return;
            }
            if (lastScaleUp != null) {
