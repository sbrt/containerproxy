...
public class AdminController extends BaseController {
    ...
    public ResponseEntity<ApiResponse<Void>> scaleApp(@PathVariable String specId, @RequestBody ScaleRequest request) {
        ...
        Integer minimumSeats = request.getMinimumSeats();
        if (minimumSeats == null) {
            ...
            int delta = minimumSeats - currentSeats;

            // Reductions are always allowed (even if currently over-allocated)
            if (delta < 0) {
                scaler.setMinimumSeatsAvailable(minimumSeats);
            } else if (delta > 0) {
                // Validate per-app max instances
                Integer appMaxInstances = spec.getMaxTotalInstances();
                if (appMaxInstances > -1 && minimumSeats > appMaxInstances) {
                    return ApiResponse.fail(
                        "Requested seats (" + minimumSeats + ") exceed per-app maximum instances limit (" + appMaxInstances + ")"
                    );
                }
                ...
                if (globalMaxInstances > -1) {
                    int totalCurrentMinimumSeats = calculateTotalMinimumSeatsAcrossAllApps();
                    int remainingCapacity = globalMaxInstances - totalCurrentMinimumSeats;
    ...
    private int calculateTotalMinimumSeatsAcrossAllApps() {
